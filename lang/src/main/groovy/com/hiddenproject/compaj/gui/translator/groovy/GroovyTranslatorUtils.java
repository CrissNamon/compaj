...
import com.hiddenproject.compaj.gui.translator.data.CodeStringData;
import groovy.lang.GroovyRuntimeException;

public class GroovyTranslatorUtils implements com.hiddenproject.compaj.gui.translator.TranslatorUtils {

  private final List<com.hiddenproject.compaj.gui.translator.CodeCheck> syntaxCheckers;
  private final List<com.hiddenproject.compaj.gui.translator.CodeTranslation> codeTranslations;
  ...
  public void addSyntaxCheck(com.hiddenproject.compaj.gui.translator.CodeCheck codeCheck) {
    syntaxCheckers.add(codeCheck);
  }

  public void addCodeTranslation(com.hiddenproject.compaj.gui.translator.CodeTranslation codeTranslation) {
  ...
  private String applyTranslations(String script) {
    for (com.hiddenproject.compaj.gui.translator.CodeTranslation codeTranslation : codeTranslations) {
  ...
  private void applySyntaxCheck(String script) {
    for (com.hiddenproject.compaj.gui.translator.CodeCheck codeCheck : syntaxCheckers) {
  ...
  private String translateGlobalMetaClassMethod(String script) {
    ...
    Matcher m = p.matcher(script);
    String tmp = script;
    Map<String, List<com.hiddenproject.compaj.gui.translator.data.MethodOverrider>> methodOverriderMap = new HashMap<>();
    while (m.find()) {
      if (isLexemeInString(m.start(), m.end())) {
        continue;
      }
      ...
      methodOverriderMap.get(m.group(1)).add(new com.hiddenproject.compaj.gui.translator.data.MethodOverrider(m.group(3), m.group(4), body));
    }
    StringBuilder scriptBuilder = new StringBuilder(script);
    for (Map.Entry<String, List<com.hiddenproject.compaj.gui.translator.data.MethodOverrider>> entry : methodOverriderMap.entrySet()) {
      for (com.hiddenproject.compaj.gui.translator.data.MethodOverrider methodOverrider : entry.getValue()) {
  ...
  private String translateOverrideMethods(String script) {
    ...
    Matcher m = p.matcher(script);
    Map<String, com.hiddenproject.compaj.gui.translator.data.ClassOverrider> extenders = new HashMap<>();
    Map<String, List<com.hiddenproject.compaj.gui.translator.data.MethodOverrider>> methodOverriderMap = new HashMap<>();
    String tmp = script;
    while (m.find()) {
      if (isLexemeInString(m.start(), m.end())) {
      ...
      extenders.putIfAbsent(m.group(1), new com.hiddenproject.compaj.gui.translator.data.ClassOverrider(m.group(1), m.group(3), m.group(2)));
      methodOverriderMap.putIfAbsent(m.group(1), new ArrayList<>());
      ...
      extenders.get(m.group(1))
          .addMethodOverrider(new com.hiddenproject.compaj.gui.translator.data.MethodOverrider(m.group(4), returnType, "{" + body));
      script = script.replace(m.group() + body, "");
    }
    StringBuilder overriders = new StringBuilder();
    for (Map.Entry<String, com.hiddenproject.compaj.gui.translator.data.ClassOverrider> entry : extenders.entrySet()) {
