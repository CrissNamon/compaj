 ...
 * MA 02111-1307 USA A copy of the GNU LGPL may be found at <code>
 * http://www.gnu.org/copyleft/lesser.html</code>
 *
...
public class PngEncoderFX extends Object {
  ...
  /**
   * Creates an array of bytes that is the PNG equivalent of the current image, specifying whether
   * to encode alpha or not.
   *
  ...
  /**
   * Creates an array of bytes that is the PNG equivalent of the current image. Alpha encoding is
   * determined by its setting in the constructor.
   *
   * @return an array of bytes, or null if there was a problem
   */
  ...
  /**
   * Write an array of bytes into the pngBytes array, specifying number of bytes to write. Note:
   * This routine has the side effect of updating maxPos, the largest element written in the array.
  ...
  /**
   * Write an array of bytes into the pngBytes array. Note: This routine has the side effect of
  ...
  /**
   * Perform "sub" filtering on the given row. Uses temporary array leftBytes to store the original
  ...
  /**
   * Write the image data into the pngBytes array. This will write one or more PNG "IDAT" chunks. In
   * order to conserve memory, this method grabs as many rows as will fit into 32K bytes, or the
   * whole image; whichever is less.
   *
